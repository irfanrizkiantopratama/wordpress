...
$sidebar_size = hootdu_layout( 'sidebar' );
...
?>
	...
	<aside <?php hoot_attr( 'sidebar', 'primary' ); ?>>
		<div <?php hoot_attr( 'sidebar-wrap', 'primary' ); ?>>
			...
			do_action( 'hootdu_sidebar_start', 'primary' );
			...
			elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
				...
				the_widget(
					...
					array(
						...
						'text'   => sprintf( __( 'This is an example widget to show how the Primary sidebar looks by default. You can add widgets to this area from the %1$swidgets screen%2$s in wp-admin.<br /><br />(This example widget is only displayed to logged in admins when no widget has yet been added to this area.)<br /><strong>Your visitors will not see this example text.<strong>', 'hoot-du' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
						'filter' => true,
					),
					array(
			...
			// Template modification Hook
			do_action( 'hootdu_sidebar_end', 'primary' );
	...
	?>
		...
		<aside <?php hoot_attr( 'sidebar', 'secondary' ); ?>>
			<div <?php hoot_attr( 'sidebar-wrap', 'secondary' ); ?>>
				...
				// Template modification Hook
				do_action( 'hootdu_sidebar_start', 'secondary' );
				...
				elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
					...
					the_widget(
						...
						array(
							'title'  => __( 'Example Widget', 'hoot-du' ),
							/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
							'text'   => sprintf( __( 'This is an example widget to show how the Secondary sidebar looks by default. You can add widgets to this area from the %1$swidgets screen%2$s in wp-admin.<br /><br />(This example widget is only displayed to logged in admins when no widget has yet been added to this area.)<br /><strong>Your visitors will not see this example text.<strong>', 'hoot-du' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
							'filter' => true,
						),
						array(
							'before_widget' => '<section class="widget widget_text">',
				...
				// Template modification Hook
				do_action( 'hootdu_sidebar_end', 'secondary' );
