 ...
 * @return void
 */
if ( !function_exists( 'hootdu_branding' ) ):
function hootdu_branding() {
	?>
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_logo' ) ):
function hootdu_logo() {

	$display = '';
	$hootdu_logo = hoot_get_mod( 'logo' );
	...
	if ( 'text' == $hootdu_logo || 'custom' == $hootdu_logo ) {
		$display .= hootdu_get_text_logo( $hootdu_logo, $tag_h1, $tag_h2 );
	} elseif ( 'mixed' == $hootdu_logo || 'mixedcustom' == $hootdu_logo ) {
		$display .= hootdu_get_mixed_logo( $hootdu_logo, $tag_h1, $tag_h2 );
	} elseif ( 'image' == $hootdu_logo ) {
		$display .= hootdu_get_image_logo( $hootdu_logo, $tag_h1, $tag_h2 );
	}

	echo wp_kses( apply_filters( 'hootdu_logo', $display, $hootdu_logo, $tag_h1, $tag_h2 ), hoot_data( 'hootallowedtags' ) );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $hootdu_logo text|custom
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'hootdu_get_text_logo' ) ):
function hootdu_get_text_logo( $hootdu_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$title_icon = hoot_sanitize_fa( hoot_get_mod( 'site_title_icon', NULL ) );

	$class = $id = 'site-logo-' . esc_attr( $hootdu_logo );
	$class .= ( $title_icon ) ? ' site-logo-with-icon' : '';
	$class .= ( 'text' == $hootdu_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
			$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
				...
				$title = ( 'custom' == $hootdu_logo ) ? hootdu_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
				$display .= apply_filters( 'hootdu_site_title', $title );
			$display .= "</a>";
		$display .= "</{$tag_h1}>";
	...
	return apply_filters( 'hootdu_get_text_logo', $display, $hootdu_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $hootdu_logo mixed|mixedcustom
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'hootdu_get_mixed_logo' ) ):
function hootdu_get_mixed_logo( $hootdu_logo, $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$has_logo = has_custom_logo();

	$class = $id = 'site-logo-' . esc_attr( $hootdu_logo );
	$class .= ( !empty( $has_logo ) ) ? ' site-logo-with-image' : '';
	$class .= ( 'mixed' == $hootdu_logo && !function_exists( 'hoot_lib_premium_core' ) ) ? ' site-logo-text-' . hoot_get_mod( 'logo_size' ) : '';
	...
	$display .= '<div id="' . esc_attr( $id ) . '" class="' . esc_attr( $class ) . '">';
		...
		$display .= '<div class="site-logo-mixed-text">';
			...
			$display .= "<{$tag_h1} " . hoot_get_attr( 'site-title' ) . '>';
				$display .= '<a href="' . esc_url( home_url() ) . '" rel="home" itemprop="url">';
					$title = ( 'mixedcustom' == $hootdu_logo ) ? hootdu_get_custom_text_logo() : '<span class="blogname">' . get_bloginfo( 'name' ) . '</span>';
					$display .= apply_filters( 'hootdu_site_title', $title );
				$display .= "</a>";
			$display .= "</{$tag_h1}>";
	...
	return apply_filters( 'hootdu_get_mixed_logo', $display, $hootdu_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @since 1.0
 * @access public
 * @param string $hootdu_logo
 * @param string $tag_h1
 * @param string $tag_h2
 * @return void
 */
if ( !function_exists( 'hootdu_get_image_logo' ) ):
function hootdu_get_image_logo( $hootdu_logo = 'image', $tag_h1 = 'div', $tag_h2 = 'div' ) {
	$display = '';
	$has_logo = has_custom_logo();
	...
	return apply_filters( 'hootdu_get_image_logo', $display, $hootdu_logo, $tag_h1, $tag_h2 );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_get_custom_text_logo' ) ):
function hootdu_get_custom_text_logo() {
	$title = '';
	$logo_custom = apply_filters( 'hootdu_logo_custom_text', hoot_sortlist( hoot_get_mod( 'logo_custom' ) ) );
	...
	return apply_filters( 'hootdu_get_custom_text_logo', $title, $logo_custom );
}
endif;
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_header_aside' ) ):
function hootdu_header_aside() {
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_secondary_menu' ) ):
function hootdu_secondary_menu( $location ) {
	...
	if ( $location == $menu_location ) {
		...
		?>
		<div <?php hoot_attr( 'header-part', 'supplementary', $menu_side_class . ' contrast-typo' ); ?>>
			<div class="hgrid">
				<div class="hgrid-span-12">
					<?php
					if ( $menu_side )
						echo '<div ' . hoot_get_attr( 'navarea-table', '', 'table' ) . '><div ' . hoot_get_attr( 'menu-nav-box', 'menu-side', 'table-cell-mid' ) . '>';
					else
						echo '<div ' . hoot_get_attr( 'menu-nav-box', 'menu-side-none' ) . '>';
					// Loads the template-parts/menu-secondary.php template.
					hoot_get_menu( 'secondary' );
					echo '</div>';
					if ( $menu_side ) {
						echo '<div ' . hoot_get_attr( 'menu-side-box', '', 'table-cell-mid inline-nav js-search' ) . '>';
 ...
 * @return void
 */
function hootdu_nav_menu_toplevel_items( $theme_location = 'hoot-primary-menu' ) {
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_menu_description' ) ):
function hootdu_menu_description( $title, $item, $args, $depth ) {
...
add_filter( 'nav_menu_item_title', 'hootdu_menu_description', 5, 4 );
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_add_custom_title_content' ) ):
function hootdu_add_custom_title_content( $location = 'pre', $context = '' ) {

	$pre_title_content_post = apply_filters( 'hootdu_pre_title_content_post', '', $location, $context );
	if ( ( $location == 'pre' && !$pre_title_content_post ) ||
		 ( $location == 'post' && $pre_title_content_post ) ) : 

		$pre_title_content = apply_filters( 'hootdu_pre_title_content', '', $location, $context );
		if ( !empty( $pre_title_content ) ) :

			$pre_title_content_stretch =  apply_filters( 'hootdu_pre_title_content_stretch', '', $location, $context ); ?>
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_display_404_content' ) ):
function hootdu_display_404_content() {
	echo esc_html( __( 'Apologies, but no entries were found.', 'hoot-du' ) );
}
endif;
add_action( 'hootdu_404_content', 'hootdu_display_404_content', 5 );
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_footer_structure' ) ):
function hootdu_footer_structure() {
 ...
 * @return int
 */
function hootdu_get_footer_columns() {
	$footers = hoot_get_mod( 'footer' );
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_get_column_span' ) ):
function hootdu_get_column_span( $col_width ) {
	$return = array();
	switch( $col_width ):
		case '100':
...
/**
 * Wrapper function for hootdu_layout() to get the class names for current context.
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_layout_class' ) ):
function hootdu_layout_class( $context ) {
	return hootdu_layout( $context, 'class' );
}
endif;
 ...
 * @return string
 */
if ( !function_exists( 'hootdu_layout' ) ):
function hootdu_layout( $context, $return = 'size' ) {
 ...
 * @access public
 */
if ( !function_exists( 'hootdu_set_layout' ) ):
function hootdu_set_layout() {
	...
	$sidebar = esc_attr( apply_filters( 'hootdu_layout', $sidebar ) );

	// Save the layout for current view
	hootdu_set_layout_span( $sidebar );
 ...
 * @access public
 */
if ( !function_exists( 'hootdu_set_layout_span' ) ):
function hootdu_set_layout_span( $sidebar ) {
	$spans = apply_filters( 'hootdu_layout_spans', array(
		'none' => array(
			'content' => 9,
			'sidebar' => 0,
		),
 ...
 * @access public
 */
if ( !function_exists( 'hootdu_thumbnail_size_contentwidth' ) ):
function hootdu_thumbnail_size_contentwidth() {
	return 'span-' . hootdu_layout( 'content' );
}
endif;
add_filter( 'hoot_thumbnail_size_contentwidth', 'hootdu_thumbnail_size_contentwidth' );
 ...
 * @access public
 */
if ( !function_exists( 'hootdu_titlearea_top' ) ):
function hootdu_titlearea_top() {
	...
	if ( in_array( 'no-sidebar', $full ) ) {
		$sidebar_size = hootdu_layout( 'primary-sidebar' );
		if ( empty( $sidebar_size ) )
			return apply_filters( 'hootdu_titlearea_top', true, 'no-sidebar', $full );
	}

	/* For Posts */
	if ( is_singular( 'post' ) ) {
		if ( in_array( 'posts', $full ) )
			return apply_filters( 'hootdu_titlearea_top', true, 'posts', $full );
		else
			return apply_filters( 'hootdu_titlearea_top', false, 'posts', $full );
	}

	/* For Pages */
	if ( is_page() ) {
		if ( in_array( 'pages', $full ) )
			return apply_filters( 'hootdu_titlearea_top', true, 'pages', $full );
		else
			return apply_filters( 'hootdu_titlearea_top', false, 'pages', $full );
	}

	/* Default */
	if ( in_array( 'default', $full ) )
		return apply_filters( 'hootdu_titlearea_top', true, 'default', $full );
	else
		return apply_filters( 'hootdu_titlearea_top', false, 'default', $full );
 ...
 * @access public
 */
if ( !function_exists( 'hootdu_loopmeta_header_img' ) ):
function hootdu_loopmeta_header_img( $context, $display ) {
	...
	$img_id = apply_filters( 'hootdu_loopmeta_header_img_id', $img_id, $context, $location, $view_id );
	$img_id = absint( $img_id );

	if ( $location == 'header' && !empty( $img_id ) ) {
		$img_src = wp_get_attachment_image_src( $img_id, apply_filters( "hootdu_{$context}_imgsize", 'full', 'header' ) );
		if ( !empty( $img_src[0] ) ) {
			$wrap_attr = array(
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_ns_filter_avatar' ) ):
function hootdu_ns_filter_avatar( $avatar, $id_or_email, $size, $default, $alt, $args ) {
 ...
 * @return void
 */
if ( !function_exists( 'hootdu_post_prev_next_links' ) ):
function hootdu_post_prev_next_links() {
	if ( hoot_get_mod( 'post_prev_next_links' ) ) {
		$display = true; $style = '';
	}
	if ( !empty( $display ) ) :
	?><div id="loop-nav-wrap" class="loop-nav"<?php echo $style; ?>><?php
		previous_post_link( '<div class="prev">' . __( 'Previous Post: %link', 'hoot-du' ) . '</div>', '%title' );
		next_post_link(     '<div class="next">' . __( 'Next Post: %link',     'hoot-du' ) . '</div>', '%title' );
	?></div><!-- .loop-nav --><?php
	endif;
}
endif;
 ...
 * @access public
 */
if ( !function_exists( 'hootdu_jetpack_infinitescroll_render' ) ):
function hootdu_jetpack_infinitescroll_render(){
