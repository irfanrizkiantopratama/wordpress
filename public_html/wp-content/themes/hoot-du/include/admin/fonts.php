 ...
 * @return void
 */
function hootdu_google_fonts_enqueue_url() {
	$fonts_url = '';
	$query_args = apply_filters( 'hootdu_google_fonts_enqueue_url_args', array() );
	...
	if ( !is_array( $query_args ) || empty( $query_args ) ):
		...
		* into your own language.
		*/
		$comfortaa = ( 'alternate' == hoot_get_mod( 'logo_fontface' ) || 'alternate' == hoot_get_mod( 'headings_fontface' ) ) ? _x( 'on', 'Comfortaa font: on or off', 'hoot-du' ) : 'off';
		$oswald = ( 'display' == hoot_get_mod( 'logo_fontface' ) || 'display' == hoot_get_mod( 'headings_fontface' ) ) ? _x( 'on', 'Oswald font: on or off', 'hoot-du' ) : 'off';
		$patua = ( 'display2' == hoot_get_mod( 'logo_fontface' ) || 'display2' == hoot_get_mod( 'headings_fontface' ) ) ? _x( 'on', 'Patua One font: on or off', 'hoot-du' ) : 'off';
		$slabo = ( 'heading' == hoot_get_mod( 'logo_fontface' ) || 'heading' == hoot_get_mod( 'headings_fontface' ) ) ? _x( 'on', 'Slabo 27px font: on or off', 'hoot-du' ) : 'off';
		...
		if ( 'off' !== $fira_sans || 'off' !== $comfortaa || 'off' !== $oswald || 'off' !== $patua || 'off' !== $slabo ) {
			...
			$query_args = apply_filters( 'hootdu_google_fonts_query_args', $query_args, $font_families );
 ...
 * @return array
 */
function hootdu_fonts_list( $fonts ) {
...
add_filter( 'hoot_fonts_list', 'hootdu_fonts_list' );