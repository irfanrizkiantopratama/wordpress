...
function hoot_attr_sidebar( $attr, $context ) {
	...
	if ( $context ) {
		...
		if ( $sidebar_name ) {
			...
			$attr['aria-label'] = esc_attr( sprintf( _x( '%s Sidebar', 'sidebar aria label', 'hoot-du' ), $sidebar_name ) );
		}
	}
...
function hoot_attr_menu( $attr, $context ) {
	...
	if ( $context ) {
		...
		if ( $menu_name ) {
			...
			$attr['aria-label'] = esc_attr( sprintf( _x( '%s Menu', 'nav menu aria label', 'hoot-du' ), $menu_name ) );
		}
	}
...
function hoot_attr_entry_published( $attr ) {
	...
	$attr['title']    = get_the_time( _x( 'l, F j, Y, g:i a', 'post time format', 'hoot-du' ) );
...
function hoot_attr_comment_published( $attr ) {
	...
	$attr['title']    = get_comment_time( _x( 'l, F j, Y, g:i a', 'comment time format', 'hoot-du' ) );
	$attr['itemprop'] = 'datePublished';
