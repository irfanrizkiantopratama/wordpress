...
function hoot_archive_title_filter( $title ) {
	...
	elseif ( is_category() )
		$title = str_replace( __( 'Category: ', 'hoot-du' ), '', $title );

	elseif ( is_tag() )
		$title = str_replace( __( 'Tag: ', 'hoot-du' ), '', $title );

	elseif ( is_tax() && !is_tax( 'post_format' ) )
		$title = single_term_title( '', false );

	elseif ( is_author() )
		$title = str_replace( __( 'Author: ', 'hoot-du' ), '', $title );

	elseif ( is_search() )
		/* Translators: %s is search query */
		$title = sprintf( esc_html__( 'Search results for: %s', 'hoot-du' ), get_search_query() );

	elseif ( function_exists( 'bbp_is_single_user' ) && bbp_is_single_user() )
		$title = __( 'User Profile ', 'hoot-du' );
	elseif ( is_post_type_archive() )
		$title = str_replace( __( 'Archives: ', 'hoot-du' ), '', $title );
	...
	if ( ( ( $page = get_query_var( 'paged' ) ) || ( $page = get_query_var( 'page' ) ) ) && $page > 1 ) {
		...
		$suffix = ' <span class="loop-title-suffix">' . sprintf( __( '(Page %s)', 'hoot-du' ), $paged ) . '</span>';
...
function hoot_untitled_post( $title ) {
	...
	if ( ! $title && ! is_singular() && in_the_loop() && ! is_admin() )
		$title = esc_html__( '(Untitled)', 'hoot-du' );
