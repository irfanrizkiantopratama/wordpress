...
function photolite_sanitize_checkbox( $checked ) {
	// Boolean check.
...
function photolite_format_for_editor( $text, $default_editor = null ) {
	...
	$wp_customize->add_control(
		new WP_Customize_Color_Control($wp_customize,'footer_color',array(
			'label' => esc_html__('Footer Background Color','photolite'),
			'description'	=> esc_html__('Select background color for footer.','photolite'),
			'section' => 'colors',
	...
	$wp_customize->add_setting('hide_slider',array(
			'default' => true,
			'sanitize_callback' => 'photolite_sanitize_checkbox',
	...
	$wp_customize->add_control( 'hide_slider', array(
    	   ...
    	   'label'     => __('Check this to hide slider','photolite'),
    	   'type'      => 'checkbox'
     ));
...
function photolite_css(){
        ...
        <style>
				...
				.copyright-wrapper{
					background-color:<?php echo esc_html(get_theme_mod('footer_color','#2a2b2c')); ?>;
				}
		</style>
	<?php }
add_action('wp_head','photolite_css');

function photolite_customize_preview_js() {
	wp_enqueue_script( 'photolite-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'photolite_customize_preview_js' );