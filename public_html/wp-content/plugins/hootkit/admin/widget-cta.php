 ...
 */
class HootKit_CTA_Widget extends HootKit_WP_Widget {

	function __construct() {

		$settings['id'] = 'hootkit-cta';
		$settings['name'] = __( 'HootKit > Call To Action', 'hootkit' );
		...
		if ( !in_array( 'cta-styles', hootkit()->get_config( 'supports' ) )
			 && !in_array( 'cta-styles', hootkit()->get_config( 'settings' ) ) // Unos <= 2.7.1 // @todo remove in future version
			)
	...
	function display_widget( $instance, $before_title = '', $title = '', $after_title = '' ) {
		...
		$widget_template = ( $widget_template ) ? $widget_template : hootkit()->dir . 'template/widget-cta.php';
