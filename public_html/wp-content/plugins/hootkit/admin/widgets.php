 ...
 * @access public
 */
if ( !class_exists( 'HootKit_WP_Widget' ) ):
abstract class HootKit_WP_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$defaults = array();
		foreach( $this->form_options as $optionid => $option ) {
			if ( isset( $option['std'] ) ) {
				$defaults[ $optionid ] = $option['std'];
			}
			...
			elseif( isset( $instance['panels_info'] ) && isset( $option['type'] ) ) {
				switch ( $option['type'] ) :
					case 'text' :       $defaults[ $optionid ] = '';      break;
					case 'textarea' :   $defaults[ $optionid ] = '';      break;
					case 'separator' :                                    break;
					case 'checkbox':    $defaults[ $optionid ] = 0;       break;
					case 'select':      foreach ( $defaults[ $option['options'] ] as $sopk => $sopv ) { $defaults[ $optionid ] = $sopk; break; } break;
					case 'multiselect': $defaults[ $optionid ] = array(); break;
					case 'smallselect': foreach ( $defaults[ $option['options'] ] as $sopk => $sopv ) { $defaults[ $optionid ] = $sopk; break; } break;
					case 'radio':
					case 'images':      foreach ( $defaults[ $option['options'] ] as $sopk => $sopv ) { $defaults[ $optionid ] = $sopk; break; } break;
					case 'icon':        $defaults[ $optionid ] = '';      break;
					case 'image':       $defaults[ $optionid ] = '';      break;
					case 'color':       $defaults[ $optionid ] = '';      break;
					case 'group':       $defaults[ $optionid ] = array(); break;
					case 'collapse':    $defaults[ $optionid ] = array(); break;
				endswitch;
			}
		}
