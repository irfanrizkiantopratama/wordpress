...
// Add Pagination
if ( !function_exists( 'hootkit_slider_postcarousel_pagination' ) ) :
	function hootkit_slider_postcarousel_pagination( $type, $instance = array() ) {
		if ( !is_string( $type ) || $type != 'postcarousel' ) return;
		global $hoot_data;
...
if ( !empty( $viewall ) ) :
	if ( !empty( $viewall ) && $viewall == 'top' ) {
		add_action( 'hootkit_carousel_start', 'hootkit_slider_postcarousel_pagination', 10, 2 );
		remove_action( 'hootkit_carousel_end', 'hootkit_slider_postcarousel_pagination', 10, 2 );
	} elseif ( !empty( $viewall ) && $viewall == 'bottom' ) {
		add_action( 'hootkit_carousel_end', 'hootkit_slider_postcarousel_pagination', 10, 2 );
		remove_action( 'hootkit_carousel_start', 'hootkit_slider_postcarousel_pagination', 10, 2 );
	} else {
		remove_action( 'hootkit_carousel_start', 'hootkit_slider_postcarousel_pagination', 10, 2 );
		remove_action( 'hootkit_carousel_end', 'hootkit_slider_postcarousel_pagination', 10, 2 );
	}
