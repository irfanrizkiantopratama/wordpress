...
<div class="profile-widget-wrap <?php echo hoot_sanitize_html_classes( "{$top_class} {$bottom_class}" ); ?>">
	<div class="profile-widget">
		...
		/* Display Title */
		if ( !empty( $title ) )
			echo wp_kses_post( apply_filters( 'hootkit_widget_title', $before_title . $title . $after_title, 'profile', $title, $before_title, $after_title ) );
		...
		if ( !empty( $image ) ) :
			...
			<div class="profile-image <?php echo 'profile-img-' . esc_attr( $img_style ); ?>">
				...
				if ( $img_style == 'full' ) {
					...
					if ( !empty( $img_src[0] ) ) echo '<div class="profile-img-placeholder" style="' . "background-image:url(" . $img_src[0] . ");" . '"></div>';
				} ?>
			</div>
			<?php
		endif;
		...
		<?php if ( !empty( $url ) ) { ?>
			...
			<?php } else { ?>
				<div class="profile-buttonlink">
					<a href="<?php echo esc_url( $url ); ?>" <?php hoot_attr( 'profile-button', 'widget', 'button button-small border-box ' ); ?>><?php echo esc_html( $button_text ); ?></a>
				</div>
			<?php } ?>
		<?php } ?>
