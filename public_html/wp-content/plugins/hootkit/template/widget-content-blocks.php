...
<div class="content-blocks-widget-wrap content-blocks-pages <?php echo hoot_sanitize_html_classes( "{$top_class} {$bottom_class}" ); ?>">
	<div class="content-blocks-widget">

		<?php
		/* Display Title */
		if ( !empty( $title ) )
			echo wp_kses_post( apply_filters( 'hootkit_widget_title', $before_title . $title . $after_title, 'content-blocks', $title, $before_title, $after_title ) );

		// Template modification Hook
		do_action( 'hootkit_content_blocks_start', 'custom', false, false, ( ( !isset( $instance ) ) ? array() : $instance ) );
		?>
					...
					<div <?php hoot_attr( 'content-block',
						...
						<?php if ( $visualtype == 'image' ) : ?>
							<div class="content-block-visual content-block-image">
								...
								if ( !empty( $imageid ) )
									echo $linktag . '<div class="entry-featured-img-wrap">' . wp_get_attachment_image( $imageid, $img_size, '', array( 'class' => "content-block-img attachment-{$img_size} size-{$img_size}", 'itemprop' => 'image' ) ) . '</div>' . $linktagend;
								?>
							</div>
						...
						<div class="content-block-content<?php
							...
							if ( !empty( $box['subtitle'] ) )
								echo '<div class="content-block-subtitle small">' . do_shortcode( wp_kses_post( $box['subtitle'] ) ) . '</div>';
		...
		<?php
		// Template modification Hook
		do_action( 'hootkit_content_blocks_end', 'custom', false, false, ( ( !isset( $instance ) ) ? array() : $instance ) );
		?>
